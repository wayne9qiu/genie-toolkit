...
const JsonDatagramSocket = require('../utils/json_datagram_socket');

const DEFAULT_QUESTION = 'translate from english to thingtalk';

class Worker extends events.EventEmitter {
    constructor(id, modeldir, split) {
        super();
    ...
    request(task, context, question, answer=undefined) {
...
module.exports = class Predictor {
    constructor(id, modeldir, nWorkers= 1, split = 'valid') {
    ...
    _startWorker() {
        const worker = new Worker(`${this.id}/${this._nextId++}`, this._modeldir, this._split);
        worker.on('error', (err) => {
    ...
    predict(context, question = DEFAULT_QUESTION, answer= undefined, task = 'almond') {
