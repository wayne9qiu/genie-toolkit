...
class ValueListLoader {
    constructor(provider, samplingType, rng) {
...
module.exports = class ParameterReplacer {
    constructor(schemas, constProvider, options) {
        ...
        this._loader = new ValueListLoader(constProvider, options.samplingType, options.rng);
    ...
    _transformValue(sentenceValue, programValue, arg) {
        ...
        if (arg && arg.metadata.pluralize && coin(0.5, this._rng)) {
            const plural = this._langPack.pluralize(sentenceValue);
            if (plural)
                return { sentenceValue: plural, programValue };
        }

        let hasDefiniteArticle = false;
        if (this._langPack.DEFINITE_ARTICLE_REGEXP) {
            // remove a definite article ("the") from the program value if we have it
            const match = this._langPack.DEFINITE_ARTICLE_REGEXP.exec(programValue);
            if (match !== null) {
        ...
        if (!hasDefiniteArticle && arg.name === 'id' && coin(0.5, this._rng)) {
            // else add it to the sentence value if the argument is an entity
            const added = this._langPack.addDefiniteArticle(sentenceValue);
            if (added)
                sentenceValue = added;
        }
    ...
    async _sampleParam(slot, replacedValuesSet) {
        ...
        while (attempts > 0) {
            ...
            // fail immediately if sampled value includes return char
            if (sampled.includes('\n')){
                attempts -= 1;
                continue;
            }
            ...
            if (operator === '=~') {
                let candidate = sampleString(words, this._langPack, this._rng);
                if (!candidate) {
                    attempts -= 1;
                    continue;
                }
            ...
            if (arg && arg.metadata.pluralize && coin(0.5, this._rng)) {
                const plural = this._langPack.pluralize(sampled);
                if (plural)
        ...
        for (let slot of program.iterateSlots2()) {
            ...
            if (isEntity(value)) {
                ...
                if (slot.type.isLocation && !this._replaceLocations)
                    continue;
                if ((slot.type.isNumber || slot.type.isMeasure || slot.type.type==='tt:phone_number') && !this._replaceNumbers)
                    continue;
